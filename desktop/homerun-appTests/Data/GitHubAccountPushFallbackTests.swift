...
struct GitHubAccountPushFallbackTests {
    ...
    private func context(remoteURL: String? = "https://github.com/acme/app.git", preferred: String? = nil) -> PushAttemptContext {
        PushAttemptContext(
            directory: directory,
    ...
    func explainsSSHRemote() async {
        ...
        let fallback = GitHubAccountPushFallback(gitClient: git, gitHubClient: gh)

        let result = await fallback.retryPush(context(remoteURL: "git@github.com:acme/app.git"))

        #expect(result == .notApplicable(AccountFallbackUseCase.inapplicableExplanation(for: "git@github.com:acme/app.git")))
    ...
    func explainsMissingCLI() async {
        let gh = StubGitHubCLIClient()
        await gh.setAvailable(false)
        let fallback = GitHubAccountPushFallback(gitClient: StubGitClient(), gitHubClient: gh)
    ...
    func pushesWithAnotherAccount() async {
        let git = StubGitClient()
        let gh = StubGitHubCLIClient()
        await gh.setAccounts(accounts)
        let fallback = GitHubAccountPushFallback(gitClient: git, gitHubClient: gh)

        let result = await fallback.retryPush(context())

        #expect(result == .succeeded(account: "acme-bot", attempts: [AccountFallbackAttempt(account: "acme-bot", failureMessage: nil)]))
    ...
    func triesPreferredAccountFirst() async {
        ...
        #expect(await gh.switchedAccounts.first == "work")
    }
    ...
    func restoresAccountAfterSuccess() async {
        ...
        #expect(await gh.switchedAccounts.last == "jamie")
    }
    ...
    func recordsSwitchFailure() async {
        let gh = StubGitHubCLIClient()
        await gh.setAccounts(accounts)
        await gh.setSwitchFailures(["acme-bot"])
        let fallback = GitHubAccountPushFallback(gitClient: StubGitClient(), gitHubClient: gh)
        ...
        #expect(result == .succeeded(
            account: "work",
            attempts: [
                AccountFallbackAttempt(account: "acme-bot", failureMessage: GitHubCLIError.switchFailed("acme-bot").message),
